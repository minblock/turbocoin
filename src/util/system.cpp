...
const char * const BITCOIN_CONF_FILENAME = "turbocoins.conf";
...
#else
    const char* pszModule = "turbocoins";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Turbocoins";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Turbocoins";
#else
    // Unix
    return pathRet / ".turbocoins";
#endif
#endif
}
